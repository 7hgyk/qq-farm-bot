...
test('鹊羽灵露在好友访问会话内定向使用，并保留服务器最终资格校验', async () => {
    ...
    const plantedLand = {
        id: 3,
        unlocked: true,
        plant: { id: 9001, name: '测试作物', phases: [{ phase: 2 }] },
    };
    ...
    require.cache[farmApiPath] = loadedModule(farmApiPath, {
        getAllLands: async () => ({ lands: [plantedLand] }),
    });
    ...
    require.cache[friendApiPath] = loadedModule(friendApiPath, {
        enterFriendFarm: async (gid) => {
            calls.push('Enter');
            return { basic: { gid, name: '好友甲' }, lands: [plantedLand] };
        },
    ...
    require.cache[warehousePath] = loadedModule(warehousePath, {
        getBag: async () => {
            calls.push('Bag');
            return { item_bag: { items: [{ id: 301103, count: 2, uid: 77 }] } };
        },
