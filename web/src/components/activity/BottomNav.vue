...
.activity-nav {
  ...
  border-top: 2px solid rgba(132, 220, 252, .68);
  background: #326ba3 url('/activity-center/stellar/nav-background.png') center top / 100% 100% no-repeat;
  box-shadow: 0 -7px 18px rgba(2, 35, 80, .3), inset 0 2px rgba(255, 255, 255, .12);
}
...
.activity-nav__item--travel { --nav-image-width: 54px; --nav-image-offset-y: 7px; }
.activity-nav__item--constellation { --nav-image-width: 68px; --nav-image-offset-y: -10px; }
.activity-nav__item--shop { --nav-image-width: 53px; --nav-image-offset-y: 8px; }
.activity-nav__item--solar { --nav-image-width: 48px; --nav-image-offset-y: 7px; }
...
.activity-nav__visual::before {
  ...
  height: 48px;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(255, 246, 153, .64), rgba(72, 159, 207, .08) 70%);
  opacity: 0;
...
button[data-active] .activity-nav__visual img {
  filter: drop-shadow(0 0 6px rgba(255, 239, 129, .8));
...
.activity-nav__badge {
  ...
  border-radius: 50%;
  background: #ff4058;
  box-shadow: 0 1px 4px rgba(120, 0, 15, .55);
}
</style>
